...
//    public Bug createBug(int taskId, String title, String description, int priorityId, Integer statusId, int assignedToId, List<MultipartFile> attachments, String name) {
    ...
    public byte[] downloadAttachment(Integer attachmentId) {
        BugAttachment att = bugAttachmentRepo.findAttachmentById(attachmentId)
                .orElseThrow(() -> new RuntimeException("Attachment not found"));

        return att.getFileData(); // @Lob field
    }
    ...
    public Bug updateBug(
        ...
        // Fetch existing bug
        Bug bug = repo.findById(bugId)
                .orElseThrow(() -> new RuntimeException("Bug not found"));

        // Update title & description
        if (title != null && !title.isEmpty()) bug.setTitle(title);
        if (description != null && !description.isEmpty()) bug.setDescription(description);
